import {addCreditTransaction, addUserCredits, deductUserCredits} from './db/user'
...
export async function generateCompletionWithCredits(messageText: string, commandModel: AbangModel, userId: string) {
  ...
  const deductCredit = deductUserCredits(BigInt(userId), price)
  const setCreditCache = setUserCreditsCache(BigInt(userId), remainingCredits)
  const addTransaction = addCreditTransaction(BigInt(userId), -price, 'used', messageText)
  ...
  try {
    ...
    deductCredit.then(() => {
      addUserCredits(BigInt(userId), price).catch((err) => console.error(`Failed to refund user credits: ${err}`))
    })
    setCreditCache.then(() => {
    ...
    addTransaction.then(() => {
      addCreditTransaction(BigInt(userId), price, 'refunded', messageText).catch((err) =>
